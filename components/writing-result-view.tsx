...
import { WritingSession, WritingSummary } from "@/lib/types";
...
export function WritingResultView({ session, summary }: { session: WritingSession; summary: WritingSummary }) {
  ...
  const exportPdf = () => {
    ...
    win.document.write(`<!doctype html><html><head><meta charset="utf-8" /><title>${escapeHtml(session.prompt.title)}</title><style>
      ...
      <section class="hero"><h1>${escapeHtml(session.prompt.title)}</h1><p>IELTS Writing Task 2 • ${escapeHtml(session.difficulty)}</p><div class="score">${report.overall}</div><p>${escapeHtml(report.scaleLabel)}</p></section>
      ...
      <section class="card"><h2>${tr ? "Senin essay'in" : "Your essay"}</h2><p>${escapeHtml(session.draftText ?? "")}</p></section>
  ...
  return (
    <main className="page-shell section">
      <div style={{ maxWidth: 920, margin: "0 auto", display: "grid", gap: "1.4rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
          <section className="card" style={{ padding: "1.1rem" }}>
            <strong style={{ display: "block", marginBottom: "0.7rem" }}>{tr ? "Senin essay'in" : "Your essay"}</strong>
        ...
        <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
          <Link href={`/app/writing/task-2?promptId=${session.prompt.id}&difficulty=${session.difficulty}&retryOf=${session.id}`} className="button button-primary" onClick={() => currentUser?.id ? void trackClientEvent({ userId: currentUser.id, event: "writing_retry", path: "/app/writing/results" }) : undefined}>

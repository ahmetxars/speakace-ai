...
function MemberBadge({ label, tone = "neutral" }: { label: string; tone?: "neutral" | "success" | "warning" | "accent" }) {
...
export function AdminPanel(props: {
  ...
  return (
    <main className="page-shell section admin-shell">
      <section className="card admin-hero">
        <div className="admin-hero-copy">
          <span className="eyebrow">Admin panel</span>
          <h1>SpeakAce control center</h1>
          <p>
            Welcome, {props.sessionLabel}. Manage members, monitor activity, create referral offers, and publish custom blog content from one place.
          </p>
        </div>
      ...
      {activeTab === "overview" ? (
        <>
          <section className="admin-stats-grid">
            ...
            <StatCard label="Paid / trial" value={`${props.overview.paidMembers} / ${props.overview.trialMembers}`} />
            <StatCard label="Weekly value" value={formatMoney(props.overview.monthlyRevenueEstimate)} hint="$3.99 weekly model" />
            <StatCard label="Requests (5m)" value={props.overview.requests5m} hint={`Last request ${formatRelativeDate(props.overview.lastRequestAt)}`} />
      ...
      {activeTab === "members" ? (
        <section className="card admin-panel-card">
          ...
          <div className="admin-filter-grid">
            ...
            />
            <select value={memberTypeFilter} onChange={(event) => setMemberTypeFilter(event.target.value)} className="admin-input">
            ...
            <select value={billingFilter} onChange={(event) => setBillingFilter(event.target.value)} className="admin-input">
                ...
                <div className="admin-member-main">
                  <div className="admin-member-head">
                    <div>
                      ...
                      {member.organizationName ? <div className="admin-muted">{member.organizationName}</div> : null}
                    </div>
                    <div className="admin-member-badges">
                      <MemberBadge label={member.memberType} />
                      <MemberBadge label={member.plan} tone={member.plan === "plus" || member.plan === "pro" ? "accent" : "neutral"} />
                      <MemberBadge label={member.billingStatus} tone={member.billingStatus === "active" || member.billingStatus === "on_trial" ? "success" : "warning"} />
                    </div>
                  </div>
                  ...
                  <div className="admin-member-foot">
                    ...
                    <strong>{member.passwordStatus === "protected" ? "Protected hash" : "No password"}</strong>
                    {member.referralCodeUsed ? <span className="admin-muted">Referral: {member.referralCodeUsed}</span> : null}
                    {member.trialEndsAt ? <span className="admin-muted">Trial ends: {formatDate(member.trialEndsAt)}</span> : null}
                  </div>
                </div>
      ...
      {activeTab === "content" ? (
        <div className="admin-grid-2">
          <section className="card admin-panel-card">
            ...
            <div className="admin-form-grid">
              <select
                ...
                onChange={(event) => setNewPost((current) => ({ ...current, language: event.target.value }))}
              >
              ...
              <select
                ...
                onChange={(event) => setNewPost((current) => ({ ...current, status: event.target.value }))}
              >
              ...
              <input
                className="admin-input"
                value={newPost.slug}
                onChange={(event) => setNewPost((current) => ({ ...current, slug: event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "-") }))}
              ...
              <input
                ...
                onChange={(event) => setNewPost((current) => ({ ...current, keywords: event.target.value }))}
                placeholder="Keywords separated by commas"
              />
              <textarea
                className="admin-textarea admin-input-span"
                value={newPost.intro}
                onChange={(event) => setNewPost((current) => ({ ...current, intro: event.target.value }))}
          ...
          <section className="card admin-panel-card">
            ...
            <div className="admin-stack-list">
              {props.customPosts.length ? props.customPosts.map((post) => (
                <div key={post.id} className="admin-list-row admin-content-row">
                  ...
                  <div className="admin-list-side">
                    ...
                    <MemberBadge label={post.status} tone={post.status === "published" ? "success" : "warning"} />
                    <button
                      type="button"
                    ...
                    >
                      {contentBusyId === post.id ? "Saving..." : post.status === "published" ? "Move to draft" : "Publish"}
                    </button>
                    <a className="button button-secondary" href={`/blog/${post.slug}`} target="_blank" rel="noreferrer">
                      Open
                    </a>
                  </div>
                </div>
          ...
          <section className="card admin-panel-card">
            <div className="admin-card-head">
              <div>
                ...
                <p className="admin-muted">Share codes with learners or teachers and open trial access automatically.</p>
              </div>
            </div>
            ...
            <div className="admin-form-grid admin-form-grid-compact">
              <input value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="Code" className="admin-input" />
              <input value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Label" className="admin-input" />
              <input value={trialDays} onChange={(event) => setTrialDays(event.target.value)} placeholder="Trial days" className="admin-input" />
              <input value={usageLimit} onChange={(event) => setUsageLimit(event.target.value)} placeholder="Usage limit (optional)" className="admin-input" />
            </div>
            <div className="admin-inline-feedback">
              ...
              {props.authActivity.map((item) => (
                <div key={item.id} className="admin-list-row">
                  ...
                  <div className="admin-list-side">
                    ...
                    <MemberBadge label={item.eventType} tone={item.eventType === "signin" ? "success" : "neutral"} />
